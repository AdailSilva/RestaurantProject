...
import com.adailsilva.model.adapter.ClienteAdapter;
...
public abstract class ClienteRepository extends AbstractEntityRepository<Cliente, Integer> {
	...
	public List<ClienteAdapter> findBetweenDatesCount(Date date1, Date date2) {
		...
		return entityManager().createQuery(
				"select new com.adailsilva.model.adapter.ClienteAdapter(date(c.dataCriacao),count(c.id)) from Cliente c where date(c.dataCriacao) BETWEEN date(:data1) AND date(:data2) group by date(c.dataCriacao)",
