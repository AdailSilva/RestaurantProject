...
public abstract class PedidoRepository extends AbstractEntityRepository<Pedido, Integer> {
	...
	public List<ProdutoAdapter> findBetweenDatesFaturamento(Date date1, Date date2) {
		return entityManager().createQuery(
				"select new com.adailsilva.model.adapter.ProdutoAdapter(sum((ip.valorUnitario*ip.quantidade)),date(ip.pedido.data),sum(ip.produto.custo * ip.quantidade),count(distinct ip.pedido.id)) "
