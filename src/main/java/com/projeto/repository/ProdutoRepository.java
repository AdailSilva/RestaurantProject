...
public abstract class ProdutoRepository extends AbstractEntityRepository<Produto, Integer> {
	...
	public List<ProdutoAdapter> findByMostSellers() {
		return this.entityManager()
				.createQuery(
						"select new com.projeto.model.adapter.ProdutoAdapter(prod , SUM(ip.quantidade)) from ItemPedido ip "
	...
	public List<ProdutoAdapter> findGroupDateDay(Date dtInicial, Date dtFinal, Produto produto) {
		return this.entityManager()
				.createQuery(
						"select new com.projeto.model.adapter.ProdutoAdapter(prod , SUM(ip.quantidade),date(ip.data)) "
	...
	// return this.entityManager()
	// .createQuery(
	// "select new com.projeto.model.adapter.ProdutoAdapter(prod ,
	...
	public List<ProdutoAdapter> findGroupDateYear(Date dtInicial, Date dtFinal, Produto produto) {
		return this.entityManager()
				.createQuery(
						"select new com.projeto.model.adapter.ProdutoAdapter(prod , SUM(ip.quantidade),MONTH(ip.data),year(ip.data)) "
								+ "from ItemPedido ip " + "join ip.produto prod " + "where ip.statusIp = 'FINALIZADO' "
