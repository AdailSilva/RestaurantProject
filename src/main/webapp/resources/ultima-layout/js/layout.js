 ...
 */
PrimeFaces.widget.Ultima = PrimeFaces.widget.BaseWidget
		.extend({
			...
			init : function(cfg) {
				...
				this.menuWrapper.children('.nano').nanoScroller({
					flash : true
				});
			},
			...
			_bindEvents : function() {
				...
				this.menuButton
						.on(
								...
								function(e) {
									$this.menuButton
											.toggleClass('menu-button-rotate');
									$this.topbarItems
						...
						.on(
								...
								function(e) {
									if ($this.isHorizontal()
											&& $this.isDesktop()) {
										var item = $(this), link = item
												.children('a'), submenu = item
												.children('ul');
				...
				$(document.body).on(
						...
						function() {
							...
							if (!$this.topbarMenuClick
								...
								$this.topbarItems.find('.active-top-menu')
										.removeClass('active-top-menu');
							}
			...
			saveMenuState : function() {
				$.cookie('ultima_expandeditems', this.expandedMenuitems
						.join(','), {
					path : '/'
				});
			},
			...
			setInlineProfileState : function(expanded) {
				if (expanded)
					$.cookie('ultima_inlineprofile_expanded', "1", {
						path : '/'
					});
				else
	...
	PrimeFaces.widget.SelectBooleanButton.prototype.check = function() {
		if (!this.disabled) {
			...
			this.jq.addClass('ui-state-active').children('.ui-button-text')
					.contents()[0].textContent = this.cfg.onLabel;
	...
	PrimeFaces.widget.SelectBooleanButton.prototype.uncheck = function() {
		if (!this.disabled) {
			...
			this.jq.removeClass('ui-state-active').children('.ui-button-text')
					.contents()[0].textContent = this.cfg.offLabel;
